...
function NavItem({ to, children, onClick, isMobile = false }) {
  ...
  return (
    <TransitionLink
      to={to}
      className={`${isActive && !isMobile ? 'text-accent' : ''} group-hover:text-bg`}
...
const Navbar = () => {
  ...
  return (
    <header className="inner bg-bg sticky top-0 z-80 flex w-full items-center justify-between pt-2 pb-2 md:bg-transparent md:pt-3 md:pb-0">
      ...
      <nav className="hidden md:block">
        <ul className="nav-link space-y-[0.5px]">
          {navItems.map((item) => (
            <li key={item.label} className="group hover:bg-accent px-1 md:pr-6">
              {item.label === 'Contact' ? (
                <a href={item.to} className="group-hover:text-bg">
                  {item.label}
                </a>
              ) : (
      ...
      <aside
        ref={panelRef}
        className={`bg-bg fixed inset-0 z-80 h-dvh w-full p-4 transition-transform duration-300 md:hidden ${
